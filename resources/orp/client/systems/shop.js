...
import * as panelsVehicleCustom from '/client/panels/vehiclecustom.js';
import * as panelsGeneralStore from '/client/panels/generalstore.js';
import * as panelsCrafting from '/client/panels/crafting.js';
...
import { syncDoors } from '/client/systems/doors.js';

alt.log('Loaded: client->systems->shop.js');
...
function startShopInterval(key, value) {
    ...
    if (!shopCheckInterval) {
        ...
        alt.log(`shop.js ${shopCheckInterval}`);
        shops.forEach(shop => {
            shop.ids.forEach(id => {
...
function shopInterval() {
    ...
    alt.log(`shop.js shopKey ${interval}`);
}
